...
export default class Counter extends Plugin {
	...
	getComponent() {
		...
		return class extends React.Component {
			...
			componentDidMount() {
				inst.registerComponent(this);
			}
			componentWillUnmount() {
				inst.unregisterComponent(this);
			}
	...
	getDecorator() {
		...
		return {
			strategy(contentBlock, callback) {
				const prevBlockLengthSum = getSumOfAllBlocksBefore(
					contentBlock.getKey(),
					api.getEditorState()
				);
...
function OverLimit(props) {
	return <span className="character-over-limit">{props.children}</span>;
}
