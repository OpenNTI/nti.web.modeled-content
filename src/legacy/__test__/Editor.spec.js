...
const getText = cmp =>
	getNode(cmp).querySelector('[contenteditable]').textContent;
...
describe('Modeled Body Content Editor', () => {
		...
		testRender()
			.then(cmps =>
				cmps.forEach(cmp => {
					...
					const a = getNode(cmp);
					const aa = Array.from(
						a.querySelectorAll('[contenteditable]')
					);
	...
	test('Base Cases: Pass a string, get a BODY string out.', done => {
		...
		testRender({ initialValue: value })
			.then(cmps =>
				Promise.all(cmps.map(x => x.pendingSetup)).then(() => cmps)
			)
			.then(cmps =>
				cmps.forEach(X => {
		...
		testRender({ initialValue: value })
			...
			.then(cmps =>
				cmps.forEach(X => {
					...
					expect(X.getValue()).toEqual([
						'<p>test</p><p>test2</p><p>test3</p><p>abc</p>',
					]);
				})
			)
			.then(done, done.fail);
