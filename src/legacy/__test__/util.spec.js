...
describe('Value Convertion', () => {
	...
	test.skip('Conversion to/from EditorState sould be consistent', () => {
		const value = getValueFromEditorState(
			getEditorStateFromValue([
		...
		const reparsedValue = getValueFromEditorState(
	...
	describe('isEmpty checks', () => {
		...
		test('Empty tags is empty', () => {
			expect(
				isEmpty('<p><span></span></p><p><strong></strong></p>')
			).toBeTruthy();
		});
		...
		test('Non empty tags are not empty', () => {
			expect(
				isEmpty('<p><span>Test</span></p><p><strong>Test</strong></p>')
			).toBeFalsy();
		});
	});
});
