...
export default class ModeledBodyContent extends React.Component {
	...
	buildContent = async (props = this.props) => {
		...
		async function process(content) {
			...
			const processed = packet.body.map(part =>
				typeof part !== 'string'
					? `<widget is="div" id="${part.guid}" data-type="${part.type}"></widget>`
					: part
			);
	...
	render() {
		...
		const props = {
			...others,
			ref: this.bodyRef,
			className: cx('modeled-content', className, {
				preview: previewMode,
			}),
		};
		...
		return React.createElement(
			'div',
			props,
			...dynamicRenderers.map(renderer =>
				renderer(React, this.renderWidget)
			)
		);
	}
